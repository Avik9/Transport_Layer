  ...
  while(1)
      ...
      else if (eventptr->evtype ==  TIMER_INTERRUPT)
	{
	  if (eventptr->eventity == A) 
	    ...
	    B_timerinterrupt();
	}// end if evtype == TIMER_INTERRUPT
      else
	{
	  printf("INTERNAL PANIC: unknown event type \n");
	}// end if evtype UNRECOGNIZED (SHOULD NEVER HAPPEN)
      free(eventptr);
    }// end while
  ...
  if (avg < 0.25 || avg > 0.75)
    {
      ...
      exit(-1);
    }// end if RNG NOT WORKING AS EXPECTED
...
void insertevent(struct event* p)
{
  ...
  if (q==NULL)    /* list is empty */
      ...
      else       /* middle of list */
	{
	  ...
	  q->prev->next=p;
	  q->prev=p;
	}// end if q == some intermediate event
    }// end else list is non-empty
}//end insertevent()
...
/************************** TOLAYER3 ***************/
/* A or B is trying to send a packet down to the network layer */
void tolayer3(int AorB,struct pkt packet)
{
  ...
  if (jimsrand() < lossprob)
	...
	printf("TOLAYER3: packet being lost\n");
      return;
    }// end if random packet loss occurrs
  ...
  if (jimsrand() < corruptprob)
    ...
    }// end if random packet corruption occurs
